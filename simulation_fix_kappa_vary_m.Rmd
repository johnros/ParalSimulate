...
## Non Linear
```{r nonlinear test}
configurations <- makeConfiguration_fixKappa(reps = 1e1, m = c(5e0, 5e1), kappa = 0.5, n = c(2e2, 4e2), lambda = NA, model = my.log.link, link = exp, sigma = 1e0, beta.maker = makeBetasRandom, beta.star.maker = BetaStarIdentity, data.maker = makeRegressionData) 
configurations %>% dim
