...
```{r  OLS run}
configurations.01 <- makeConfiguration_fixKappa(
  reps = 5e1, 
  m = .m, 
  n = .n , 
  kappa = .kappa, 
...
MSEs.01 <- parApply(cl, configurations.01, 1, replicateMSE)
# stopCluster(cl)
```


```{r OLS plot, eval=FALSE, echo=FALSE}
MSEs.framed.01 <- frameMSEs(MSEs.01, configurations.01)
plotMSEs_fixKappa(MSEs.framed.01, 'test', legend.position='right')
```

```{r OLS saving}
save(MSEs.01, configurations.01, file='RData/MSEs.01.RData')
```
...
```{r nonlinear run}
configurations.30 <- makeConfiguration(reps = 1e2, m = c(5e0, 5e1), p = 5e1, n = seq(1e2, 2e2, length.out=5), kappa = 1, lambda = NA, model = my.log.link, link = exp, sigma = 1, beta.maker = makeBetasRandom, beta.star.maker = BetaStarIdentity) 
...
MSEs.framed.30 %>% select(n, average, std.dev, median, mad) %>% head
plotMSEs_fixKappa(MSEs.framed.30, 'test', robust=TRUE, legend.position='right')
```
...
```{r logistic run}
configurations.40<- makeConfiguration(reps = 1e1, m = c(5e0, 5e1), p = 1e2, n = seq(1e2, 1e3, length.out = 4), model = my.logistic, link = sigmoid, sigma = 1e1, beta.maker =makeBetasRandom, beta.star.maker = BetaStarIdentity, data.maker = makeClassificationData) 
# cl <- makeCluster(min(30, nrow(configurations.40)))
# clusterEvalQ(cl, library(InformationAndInference))
...
plotMSEs_fixKappa(MSEs.framed.40, 'test', robust=TRUE, legend.position='right')
```
