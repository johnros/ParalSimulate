...
configurations <- makeConfiguration(reps = 2e1, m = c(5e0, 1e2), p = 5e1, n = c(1.5e2, 5e2) , lambda = 1e1, model = my.ridge, link = identity, sigma = 1e1, beta.maker=makeBetasDeterministic, beta.norm = 1e1, beta.star.maker = BetaStarRidge, data.maker = makeRegressionData, name='ridge' ) 
configurations %>% dim
...
plotMSEs(MSEs.framed.1, center='bias.mean', the.title = 'Bias', y.lim=c(-1,4))
