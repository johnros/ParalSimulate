...
## OLS
```{r OLS testing}
configurations <- makeConfiguration(reps = 2e1, m = c(5e0, 5e1), p = 1e2, n = c(1.5e2, 2e2) , kappa = 1, lambda = 10, model = my.ols, link = identity, sigma = 1e1, beta.maker = makeBetasDeterministic, beta.star.maker = identityBeta, data.maker = makeRegressionData) 
configurations %>% dim
...
configurations <- makeConfiguration(reps = 2e1, m = c(5e0, 1e2), p = 5e1, n = c(1.5e2, 2e2) , kappa = 1, lambda = 1e2, model = my.ridge, link = identity, sigma = 1e1, beta.maker=makeBetasRandom, beta.star.maker  = ridgeBeta, data.maker = makeRegressionData ) 
configurations %>% dim
...
## Non Linear
```{r nonlinear test}
configurations <- makeConfiguration(reps = 1e1, m = c(5e0, 5e1), p = 5e1, n = c(1e2, 2e2) , kappa = 1, lambda = 10, model = my.log.link, link = exp, sigma = 1e0, beta.maker = makeBetasRandom, beta.star.maker = identityBeta, data.maker = makeRegressionData) 
configurations %>% dim
...
```{r nonlinear run}
configurations.3 <- makeConfiguration(reps = 1e2, m = c(5e0, 5e1), p = 5e1, n = seq(1e2, 2e2, length.out=5), kappa = 1, lambda = NA, model = my.log.link, link = exp, sigma = 1, beta.maker = makeBetasRandom, beta.star.maker = identityBeta) 
...
## Logistic Regression
```{r logistic test}
configurations <- makeConfiguration(reps = 1e1, m = c(5e0, 5e1), p = 2e1, n = c(1e2, 2e2,4e2) , kappa = 1, lambda = 10, model = my.logistic, link = sigmoid, sigma = 1e1, beta.maker =makeBetasDeterministic, beta.star.maker = identityBeta, data.maker = makeClassificationData) 
configurations %>% dim
...
plotMSEs(MSEs.framed.4, 'test', y.lim=c(0.8,2))
plotMSEs(MSEs.framed.4, 'test', y.lim=c(0.8,2), robust = TRUE)
```


```{r logistic run}
configurations.4 <- makeConfiguration(reps = 1e1, m = c(5e0, 5e1), p = 2e1, n = c(1e2, 2e2,4e2) , kappa = 1, lambda = 10, model = my.logistic, link = sigmoid, sigma = 1e1, beta.maker =makeBetasDeterministic, beta.star.maker = identityBeta, data.maker = makeClassificationData) 
