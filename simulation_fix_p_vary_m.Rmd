...
## Non Linear
```{r nonlinear test}
configurations <- makeConfiguration(reps = 1e1, m = c(5e0, 5e1), p = 5e1, n = c(1e2, 2e2) , kappa = 1, lambda = 10, model = my.log.link, link = exp, sigma = 1e0, beta.maker = makeBetasRandom, beta.star.maker = BetaStarIdentity, data.maker = makeRegressionData) 
configurations %>% dim
...
## Logistic Regression
```{r logistic test}
configurations <- makeConfiguration(reps = 1e1, m = c(5e0, 5e1), p = 2e1, n = c(1e2, 2e2,4e2) , kappa = 1, lambda = 10, model = my.logistic, link = sigmoid, sigma = 1e1, beta.maker =makeBetasDeterministic, beta.star.maker = BetaStarIdentity, data.maker = makeClassificationData) 
configurations %>% dim
...
```{r logistic run}
configurations.4 <- makeConfiguration(reps = 1e1, m = c(5e0, 5e1), p = 2e1, n = c(1e2, 2e2,4e2) , kappa = 1, lambda = 10, model = my.logistic, link = sigmoid, sigma = 1e1, beta.maker =makeBetasDeterministic, beta.star.maker = BetaStarIdentity, data.maker = makeClassificationData) 
