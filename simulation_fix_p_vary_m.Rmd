...
## OLS
```{r Testing}
configurations <- makeConfiguration(reps = 5e1, m = c(5e0, 5e1), p = 1e2, n = c(1.5e2, 2e2) , kappa = 1, lambda = 10, model = my.ols, link = identity, sigma = 1e0, beta.maker = makeBetasDeterministic, beta.star.maker = identityBeta) 
configurations %>% dim
...
plotMSEs(MSEs.framed.0, 'test', y.lim=c(0.9,2))
```
...
configurations <- makeConfiguration(reps = 4e1, m = 1e1, p = 5e1, n = c(1.1e2, 5e2) ,  kappa = 5e-1, lambda = 2, model = my.ridge, link = identity, sigma = 1e1, beta.maker =makeBetasDeterministic, beta.star.maker  = ridgeBeta ) 
...
plotMSEs(MSEs.framed.1, 'test', y.lim=c(0,3))
```
