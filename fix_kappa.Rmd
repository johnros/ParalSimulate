...
## OLS
```{r OLS testing}
configurations <- makeConfiguration(reps = 2e1, m = c(5e0, 5e1), n = c(1.5e2, 1e3) , kappa = 0.1, model = my.ols, link = identity, sigma = 1e1, beta.maker = makeBetasRandom, beta.star.maker = BetaStarIdentity, data.maker = makeRegressionData, name='ols') 
...
## Non Linear
```{r nonlinear test}
configurations <- makeConfiguration(reps = 1e1, m = c(5e0, 2e1), kappa = 0.5, n = c(2e2, 4e2), lambda = NA, model = my.log.link, link = exp, sigma = 1e0, beta.maker = makeBetasRandom, beta.star.maker = BetaStarIdentity, data.maker = makeRegressionData, name='nls') 
configurations %>% dim
...
MSEs.framed.4 %>%  plotMSEs_fixKappa('test', y.lim=c(1,20))
```
