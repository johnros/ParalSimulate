...
# load('RData/MSEs_choose_m.2.RData')
MSEs.framed.000 <- frameMSEs(MSEs.000, configurations.000)

plot.000 <- plotMSEs2(MSEs.framed.000, '(a) OLS MSE', robust=NA, legend.position='right', y.lim=c(-1,10), jitter=0, line=TRUE, fix='Np', center='MSE', bias.fixp = FALSE)
plot.000

plot.0001 <- plotMSEs2(MSEs.framed.000, '(b) OLS Bias', robust=NA, legend.position='right', y.lim=c(0,0.5), jitter=0, line=TRUE, fix='Np', center='bias.mean', bias.fixp = TRUE)
plot.0001
```
...
plot.0010 <- plotMSEs2(MSEs.framed.001, '(c) Ridge MSE', robust=NA, legend.position='right', y.lim=c(0,10), jitter=0, line=TRUE, fix='Np', center='MSE')
plot.0010

plot.0011 <- plotMSEs2(MSEs.framed.001, '(d) Ridge bias', robust=NA, legend.position='right', y.lim=c(0,2), jitter=0, line=TRUE, fix='Np', center='bias', bias.fixp = TRUE)
plot.0011
```
