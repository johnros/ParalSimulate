...
configurations.1 <- makeConfiguration(
  ...
  beta.maker = .beta.maker, beta.star.maker = BetaStarRidge,
  data.maker=makeRegressionData,
  name='ridge') 
## Non Linear
...
```{r split results}
# load(file='RData/MSEs_fix_p.8.RData') 
...
plot.3 <- plotMSEs(MSEs.framed.3, '(c) NLS', center='median', legend.position='none', jitter=.jitter, y.lim=.ylim2)

plot.4 <- plotMSEs(MSEs.framed.4, '(d) Logistic', center='median', legend.position='none', jitter=.jitter, y.lim=.ylim2)
